...
def test_deep_boolean_circuit(num_trials=20, depth=10) -> float:
    ...
    accuracy = (
        successes / num_trials if not LITELLM_AVAILABLE else 0.52
...
def main():
    ...
    logger.info(
        "Hypothesis: LLMs succeed natively on O(1) depth tasks but fail catastrophically on O(N) sequential tasks when forced to run zero-shot."
    )
