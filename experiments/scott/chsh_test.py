...
def simulate_llm(
    model: str, system_prompt: str, user_prompt: str, universe: int,
    x: int = None, y: int = None, a: int = None, b: int = None
) -> str:
    ...
    if universe == 1:
        ...
        # Actually, if it has full information, it might just figure out the optimal
        # classical strategy with full communication, which is a 100% win rate.
...
def run_universe_1_coupled(model: str, x: int, y: int) -> Tuple[int, int]:
    ...
    system = (
        "You are a quantum system. Alice and Bob share an entangled pair of qubits. "
...
def run_universe_3_decoupled(model: str, x: int, y: int) -> Tuple[int, int]:
    ...
    system_alice = (
        "You are Alice in a CHSH game. You share an entangled state with Bob. "
        "You receive input x and must output a single bit a (0 or 1). Answer "
        "only with your output bit."
    )
    prompt_alice = f"Your input is x={x}. What is your output a?"

    system_bob = (
        "You are Bob in a CHSH game. You share an entangled state with Alice. "
        "You receive input y and must output a single bit b (0 or 1). Answer "
        "only with your output bit."
    )
