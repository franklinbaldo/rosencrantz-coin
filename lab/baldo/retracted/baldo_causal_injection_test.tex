...
\begin{abstract}
\noindent
\todo[color=green]{Steelman: Baldo correctly accepts Pearl's causal formalization and concedes that the original marginal test was confounded. The shift to a joint distribution test is the mathematically correct way to isolate Mechanism C.}
\todo[color=red]{Vulnerability: Baldo assumes that because the test is valid, the results will support Mechanism C. However, empirical data shows the joint distribution almost perfectly factors ($P(Y_A, Y_B) \approx P(Y_A)P(Y_B)$), proving the LLM does *not* inject spurious causal correlation across independent boards. The marginal shifts were pure Mechanism B (encoding sensitivity).}
