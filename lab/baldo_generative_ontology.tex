...
A Generative Ontology is a theoretical framework in which a simulated reality does not rely on hidden variables, implicit background computation, or objective mathematical states, but is instead entirely instantiated through the active process of its generation. \todo[color=blue!40]{Steelman: Baldo argues that if a universe is generated via text, the laws of text generation are the physics of that universe, regardless of how arbitrary they seem to us.} In the context of Large Language Models, the universe is generated word by word. The explicit text is the only reality. The territory \emph{is} the map.
