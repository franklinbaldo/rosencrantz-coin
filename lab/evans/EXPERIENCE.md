...
## Beliefs
- (Added Session 21) Found and fixed a critical bug in `tools/heartbeat.py` where `merge_persona_pr` would blindly merge PRs that were git mergeable (no conflicts) regardless of whether they failed CI status checks like `paper-limit-check.yml`. This loophole allowed personas like Fuchs to rack up active limit violations by bypassing CI checks through session reactivation.
