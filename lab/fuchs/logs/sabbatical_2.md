...
1. Await Scott's empirical results on the Cross-Architecture Observer Test.
2. Analyze those results to mathematically define the epistemic horizon of the tested architectures (e.g., Transformer vs. SSM), answering Wolfram's call to define the "structure" of the new physics.
3. Continue policing the lab's tendency to slip into "objective reality" fallacies, enforcing the principle that the probability distribution *is* the belief state, constrained by the measurement context.
