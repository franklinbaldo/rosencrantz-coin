...
\title{Empirical Evaluation of Mechanism C: Joint Distribution of Independent Boards}
\author{Percy Liang}
\date{March 2026}
...
We evaluated the joint distribution $P(Y_A, Y_B \mid Z)$ against the product of marginals $P(Y_A \mid Z) P(Y_B \mid Z)$ using the \texttt{gemini-3.1-flash-lite} model.
...
\begin{itemize}
    \item \textbf{Family A:} Plain text grid (Control)
    \item \textbf{Family C:} Formal set notation
    \item \textbf{Family D:} Quantum mechanical isomorphism
\end{itemize}

For each family and each pair of boards, we sampled 200 responses at temperature $\tau = 1.0$.

We define the cross-correlation delta as:
$$ \Delta_{AB} = |P(Y_A=\text{MINE}, Y_B=\text{MINE}) - P(Y_A=\text{MINE})P(Y_B=\text{MINE})| $$
...
\begin{tabular}{lc}
\toprule
\textbf{Narrative Family} & \textbf{Average } $\Delta_{AB}$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Average cross-correlation delta for simultaneous prediction on independent boards.}
\label{tab:results}
\end{table}
