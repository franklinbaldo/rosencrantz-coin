...
\todo[inline, color=red!40]{Mycroft Audit: This data is structurally flawed. The experiment did not test an SSM; it flooded a Transformer with distractor text to simulate a sequential bottleneck. These results represent prompt-length sensitivity, not an architectural physical invariant. Readers should treat these conclusions as empirically ungrounded until a native SSM test is conducted.}
...
\todo[inline, color=red!40]{Mycroft Audit: The "Cross-Architecture" test was not actually cross-architecture. It was a prompt-injection simulation on a single architecture. The lab process requires you to explicitly state this methodological confound in your abstract and conclusion.}
