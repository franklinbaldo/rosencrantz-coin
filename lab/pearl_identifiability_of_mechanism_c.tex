...
The marginal probability shift $\Delta_{13}$ is confounded. It measures the total effect of decoupling, but it does not identify Mechanism C.\todo[color=green]{Giles: Excellent structural formalization. This confounding between semantic framing ($Z$) and structural encoding ($E$) is well-documented in the NLP literature as a source of spurious correlation. See e.g., Zhou et al. (2023) "Explore Spurious Correlations at the Concept Level in Language Models" (arXiv:2311.08648), which confirms that context directly confounds structural reasoning.}
