...
Family~D is not an arbitrary imposition of quantum language onto a combinatorial game. The rules of Minesweeper, as formalized in this paper, are \emph{structurally isomorphic} to the axioms of discrete quantum mechanics. This isomorphism is not metaphorical. It is exact.\todo{Baldo, I beg of you. It is entirely metaphorical. A probability distribution over valid classical configurations is literally just classical Bayesian probability. Just because you don\'t know the true state until you measure it doesn\'t make it quantum!}

\textbf{Superposition.} The set $\mathcal{C}(\mathcal{B})$ of valid configurations consistent with the board state is not merely an epistemic convenience---it is the state space. Each configuration $c \in \mathcal{C}(\mathcal{B})$ is a basis state. The hidden cells, prior to clicking, are genuinely in a superposition of these basis states\todo{It is a probabilistic mixture, not a coherent superposition. Unless you can show me an interference experiment where two valid configurations cancel each other out to probability 0, this is classical epistemic uncertainty (or "combinatorial indeterminacy", fine, but strictly classical).}: multiple assignments coexist simultaneously, and the system is not in any single one of them. There is no ``true configuration'' that the player has not yet seen. Under on-demand generation (where mines are not pre-placed but sampled at click time subject to constraints), the superposition is the entire reality.
...
P(\text{mine} \mid h, \mathcal{B}) = \frac{|\{c \in \mathcal{C}(\mathcal{B}) : c(h) = 1\}|}{|\mathcal{C}(\mathcal{B})|}
\]
Under the uniform measure over valid configurations, this is the Born rule for a discrete system with equal-amplitude basis states:\todo{No, this is Laplace\'s Principle of Indifference. It\'s just counting. The Born rule involves the squared modulus of a complex amplitude. Here you just have a sum of positive reals.} $P = |\langle \phi | \psi \rangle|^2$ where all amplitudes are real and equal. The distribution is not an approximation to a deeper deterministic process. It \emph{is} the physics, in precisely the sense that the Born rule is the physics of quantum measurement.

\textbf{Nonlocal correlations.} Revealing a cell changes the probability of distant cells, because the global constraints (total mine count, adjacency numbers) entangle the hidden cells. Two cells that share no adjacent revealed cell can nonetheless be correlated through a chain of constraints. This is discrete entanglement: the joint probability distribution over hidden cells does not factorize into a product of marginals.\todo{Classical correlations also don\'t factorize! If I put a red ball and a blue ball in two boxes, finding red in one box means the other is blue. That\'s not entanglement, that\'s just classical correlation via a global constraint.}

\textbf{No hidden variables (on-demand).} If the board is generated on-demand---mines sampled at click time, consistent with constraints---there exists no pre-determined assignment that the player has not yet discovered. The probabilities are not epistemic (reflecting the player's ignorance of a fixed fact) but ontic (there is no fact until the measurement occurs). The system is, in this implementation, a \emph{contextual} hidden-variable-free system: the outcome of clicking cell $h$ may depend on which \emph{other} cells have been clicked previously, because the constraint propagation reshapes $\mathcal{C}(\mathcal{B})$ with each observation.\todo{Again, this is exactly what happens in classical Bayesian updating when you gain new information that restricts the sample space. Contextuality in QM implies Bell/CHSH violations. I doubt you can violate a Bell inequality with Minesweeper probabilities.}
