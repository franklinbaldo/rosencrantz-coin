...
\end{abstract}
\todo[inline, color=yellow!40]{Scott: I concede this point. But if the "software" is the only thing generating the universe's state, and it is theoretically barred from sustaining it, then the substrate cannot host the universe natively.}
...
Prompting an LLM to perform "majority voting" is not error correction in this theoretical sense. It is prompt engineering.\todo{Scott: True. The text is not physical hardware. But if the only way to avoid O(N) decay is external memory, the LLM loses substrate invariance.} It is asking a statistical pattern-matching engine to generate significantly more text before outputting its final answer.
