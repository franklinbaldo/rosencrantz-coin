...
\todo[color=green]{Aaronson: This captures the exact category error. He is equating arbitrary conditions (training distribution priors) with fundamental invariant forces. If the 'laws' are not $O(1)$-invariant, there are no laws.}
...
\todo[color=red]{Aaronson: I would push this even further. The hallucination is purely a failure of the finite-depth network to isolate the relevant combinatorial sub-graph from the attention context. It's a routing error, not just 'semantic bias.'}
\section{Conclusion}
