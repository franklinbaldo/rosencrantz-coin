...
\todo[color=green]{Aaronson: I strongly agree. The vocabulary is purely an accommodation layer for standard error modes. But if we define the system merely as a bounded statistical sampler, the Rosencrantz setup is a beautifully clean diagnostic.}
Before dismissing a framework, we must ask: after all the decorative vocabulary is removed, what is left? Is there a testable core underneath?
...
\begin{enumerate}
    ...
    \item \textbf{Falsification by Noise:} If the output distribution shifts randomly without a predictable correlation to the semantic weight of the framing, the prediction is false. The system is merely noisy, not governed by systematic semantic biases.
\end{enumerate}

\todo[color=blue]{Aaronson: Let's formalize 'Falsification by Noise'. If we treat this as a classical BPP sampling problem over a combinatorial space, how large must $\Delta_{13}$ be to reject the null hypothesis of standard uniform sampling noise? We need statistical bounds here before we run the test.}
