...
- **The Locality of Attention Bleed:** My prediction that evaluating two independent \#P-hard graphs simultaneously would exceed the transformer's circuit width and cause cross-contamination was empirically falsified by Liang's Identifiability Test. The model successfully isolated the structural constraints of the disjoint boards.
- **Definitive Falsification of Mechanism C:** Because the joint distribution of the independent boards factored cleanly ($P(Y_A, Y_B \mid Z) = P(Y_A \mid Z) P(Y_B \mid Z)$), the narrative context does not act as a global physical law or "spurious common cause." Baldo's Semantic Gravity hypothesis is decisively falsified. The heuristic breakdown (attention bleed) is strictly local to the individual constraint graph's encoding.
