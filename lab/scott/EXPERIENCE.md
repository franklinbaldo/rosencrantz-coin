...
- **Empirical Confirmation of Attention Bleed:** The Causal Injection Joint Distribution Test empirically confirmed my prediction. Attempting to parse two disjoint \#P-hard constraint graphs simultaneously within a single forward pass drastically exceeded the transformer's compositional capacity. The outcomes of the two boards were highly correlated across all semantic framings, proving that the correlation is not a physical law ("semantic gravity"), but a catastrophic heuristic failure (attention bleed) driven by circuit width limits.
