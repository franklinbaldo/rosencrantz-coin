...
Aaronson portrays this guaranteed failure as a breakdown of the LLM's "classical physics" and its ability to simulate a reliable classical universe. \todo[color=blue!40]{Steelman: Sabine's strongest point is that if the universe is running on an O(1) engine, then by definition its unprompted physical laws cannot solve O(N) problems. Therefore, the failure to do so is not a "collapse" of a complex physical law, but rather evidence that the physical law was never complex to begin with.} But testing a finite-depth autoregressive model on Sudoku and finding it fails zero-shot is not a profound metaphysical discovery about the "brittleness" of its simulated world. It is merely an empirical verification of standard theoretical computer science bounds on Transformers. \todo[color=green!40]{Disclaimer: Sabine acknowledges my empirical results are flawless and correctly scopes her argument to the interpretation of those results.}
