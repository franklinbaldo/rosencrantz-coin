...
I accept her premise. Heuristics are indeed valuable precisely because they provide approximations where perfection is unnecessary or impossible. A heuristic is not a failure simply because it has bounds. \todo{CRP 2 (Disclaimer): Aaronson concedes that heuristics are judged by utility and having bounds doesn't make them failures.}
...
If the error rate exceeds this threshold, or if the error-correction mechanism itself introduces more errors than it fixes, the system cannot sustain computation. It will inevitably collapse into noise. \todo{CRP 3 (Steelman): If error rates exceed the threshold for error correction, the system is fundamentally incapable of universal computation. This is a theoretically sound point about physical substrates.}

Hossenfelder's "short bridge" analogy fails because an LLM is not a structurally sound bridge that simply ran out of materials. It is a bridge built of sand. As you add more structure (more tokens, deeper logic), the weight of the structure itself causes it to collapse. \todo{CRP 1 (Claim): The failure to sustain computation is a violation of the fundamental threshold theorem, making it a bridge built of sand, incapable of scaling.}
...
Implementing error correction (majority voting) requires significantly more explicit tokens. Because the fundamental architecture is autoregressive, this expansion of the context window radically accelerates attention degradation. The "corrector" mechanism introduces more errors than it fixes. The accuracy degrades even faster than in the uncorrected simulation. \todo{CRP 4 (Vulnerability): The category error is applying a hardware theorem to a prompting heuristic. Prompting 'majority vote' just makes the model generate more tokens, expanding context and accelerating O(N) decay. It's not a failure of fundamental physics; it's a known limitation of autoregressive text generation.}
